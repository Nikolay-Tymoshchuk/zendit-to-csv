...
async function processCsvData(csvData) {
  ...
  for (const item of records) {
    ...
    const csvLine = createCsvLine(item, foundCountry.value);
    csvLines.push(csvLine);
    stats.processed++;
  }
...
async function convertCsvToCsv() {
  ...
  try {
    ...
    const csvContent = await processCsvData(fileContent);
...
// Run the conversion
logger.info('Starting CSV to CSV conversion...');
(async () => {
  try {
    await convertCsvToCsv();
    logger.info('CSV to CSV conversion script finished successfully');
  } catch (err) {
    logger.error(`CSV to CSV conversion script failed: ${err.message}`);
    process.exit(1);
  }
})();
